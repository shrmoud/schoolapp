...
def load_mandatory_dummy_data(*args):
    ...
    loaddata(
        'class_status.json',
        'universities_production.json',
        'subjects_production.json'
    )
...
def rundaphne():
    """
    Starts the development daphne server inside the Vagrant VM.
    """
    with virtualenv():
        run(
            'daphne tandlr.wsgi:channel_layer --port 8888 --bind 192.168.33.40'
        )
...
def celery():
    ...
    """
    with virtualenv():
        run('celery -B -A tandlr worker -l info')
